...
class Board(QGraphicsView):
    ...
    def stageEnd(self):
        ...
        if self.numOfPlayers == 1:
            firstPlayerDetails = self.playerWrappers[self.playerData.firstPlayerDetails.id].getPlayerDetails()
            firstPlayerTankDetails = self.playerWrappers[self.playerData.firstPlayerDetails.id].separateTankDetails
        else:
            firstPlayerDetails = self.playerWrappers[self.playerData.firstPlayerDetails.id].getPlayerDetails()
            firstPlayerTankDetails = self.playerWrappers[self.playerData.firstPlayerDetails.id].separateTankDetails
            secondPlayerDetails = self.playerWrappers[self.playerData.secondPlayerDetails.id].getPlayerDetails()
            secondPlayerTankDetails = self.playerWrappers[self.playerData.secondPlayerDetails.id].separateTankDetails
        playerWrapper: PlayerWrapper
        for playerWrapper in self.playerWrappers.values():
